...
def main() -> None:
    ...
    """
    if len(sys.argv) < 2:
        msg = ("Usage: python3 a_maze_ing.py config.txt. "
               "Make sure config.txt included in Makefile.")
        sys.exit(msg)
    ...
    try:
        ...
        # Generate Maze with seed as an optional argument
        generator: MazeGenerator = MazeGenerator(grid,
                                                 seed=config.seed)
        generator.generate()
